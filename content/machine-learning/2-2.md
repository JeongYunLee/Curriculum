...
#### 2. 2. 1. 정확도(Accuracy)
![accuracy](static\machine-learning\2-2\2-2-1_accuracy_lt.PNG)
...
#### 2. 2. 3. **정밀도Precision, 재현율Recall(TPR), 특이도Specificity, 오류율**

전체 예측한 것 중에 예측이 맞은 것이 정확도accuracy에 해당했습니다. 
...
아니 그러면 오차 행렬은 왜 계산한 것이냐고 묻고 싶을 수 있습니다. 오차 행렬에서 계산된 TN, FP, NF, TP를 잘 조합해 **정밀도Precision**와 **재현율Recall, 특이도, 오경보율, F1 score, ROC curve(AUC)** 등을 구해 모델의 성능을 평가할 수 있기 때문입니다.


+ **TP, TN, FP, FN 도식화**

![Confusion_Matrix](static\machine-learning\2-2\2-2-2_1_confusion_matrix.PNG)

**1. 정확도(Accuracy)** = **(TP + TN) / (TN + FP + FN + TP) = (P/N 모두 정답으로 예측T) / (전체 예측)**: P든 N이든 전체 예측 중에 맞춘 비율
![accuracy](static\machine-learning\2-2\2-2-1_accuracy_lt.PNG)
![accuracy](static\machine-learning\2-2\2-2-3_1_accuracy.PNG)

**2. 정밀도Precision** = **TP/(FP+TP)**: 예측을 Positive(1) 로 한 대상 중에 예측과 실제 값이 Positive(1)로 일치한 데이터의 비율 (precision_score())
![precision_math](static\machine-learning\2-2\2-2-2_2_precision_lt.png)
![precision](static\machine-learning\2-2\2-2-3_2_precision.PNG)
...
**3. 재현율Recall** = **TP/(FN+TP)**: 실제 값이 Positive인 대상 중에 예측과 실제 값이 Positive로 일치한 데이터의 비율 (recall_score()) 
![recall_math](static\machine-learning\2-2\2-2-2_3_recall_lt.png)    
![recall](static\machine-learning\2-2\2-2-2_3_recall_lt.png)    
...
![specificity_math](static\machine-learning\2-2\2-2-2_5_specificity_lt.png)
![specificity](static\machine-learning\2-2\2-2-3_4_specificity.png)    
    
**5. 오경보율FPR(FalsePositiveRate)**: 찾고자 하는 것이 아닌 0에 대해 1이라고 **얼마나 잘못 예측 했는지를 나타내는 지표**. 0에 가까울수록 좋은 모델. 민감도가 커지면 FP가 커질 수 있으며 오경보율 또한 커지게 됩니다. **이 관계를 곡선으로 나타낸 것이 아래 서술할 ROC curve**입니다.
![FalsePositiveRate_math](static\machine-learning\2-2\2-2-2_6_false_positive_rate_lt.png)
![FalsePositiveRate](static\machine-learning\2-2\2-2-3_5_false_positive_rate.png.PNG)



#### 2. 2. 4. **F1 Score(F-measure)**
* 정밀도와 재현율을 결합한 지표
* 정밀도와 재현율이 어느 한쪽으로 치우치지 않을 때 높은 값을 가짐
![F1_math](static\machine-learning\2-2\2-2-4_1_f1_lt.png)
...
![f1_trade_off](static\machine-learning\2-2\2-2-4_2_f1_precision_recall_trade-off.png)
...
AUC는 아까 만든 ROC curve에서 아래쪽 면적을 의미합니다. (적분 값)
 ...
 ![ROC-AUC](static\machine-learning\2-2\2-2-5_1_ROC-AUC.png)
...
True Positive**TP**는 실제로 이 사람은 암에 걸려있고, 판단을 암에 걸렸다고 판단한 것이고, False Positive**FP**는 실제로는 암에 걸리지 않았는데도 암에 걸렸다고 ‘잘못’ 판단한 것을 의미합니다.
 ...
 ![ROC-AUC1](static\machine-learning\2-2\2-2-5_2_roc_auc1.png)
...
![ROC-AUC2](static\machine-learning\2-2\2-2-5_3_roc-auc2.gif)
...
![ROC-AUC3](static\machine-learning\2-2\2-2-5_4_roc-auc3.gif)
...
![ROC-AUC4](static\machine-learning\2-2\2-2-5_5_roc-auc4.gif)
...
![ROC-AUC5](static\machine-learning\2-2\2-2-5_6_roc-auc5.gif)
...
- 이미지 출처: [공돌이의 수학정리 노트]("https://angeloyeo.github.io/2020/08/05/ROC.html)